...
class EarthModelData {
    ...
    init() {
        ...
        let startRotationH = Float(0.0)
        let endRotationH = Float.pi * 2.0
        let startRotationV = Float(0.0)
        let endRotationV = Float.pi
        var indexV = 0
        while indexV <= Self.tileCountV {
            ...
            while indexH <= Self.tileCountH {
                ...
                _points[indexH][indexV].x = point.x
                _points[indexH][indexV].y = point.y
                _points[indexH][indexV].z = point.z
